        ...
        <div className="mb-2">
          {!discount ? (
            ...
            <div className="bg-orange-50 border-2 border-orange-300 rounded-xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold text-white bg-orange-500 px-2 py-0.5 rounded-full leading-none">
                    🏷️ 全体割引を適用中
                  </span>
                  {discountAmount > 0 && (
                ...
                <span className="text-[10px] text-orange-400">{discountOpen ? "▲" : "▼"}</span>
              </button>
                ...
                <div className="border-t border-orange-200 px-3 py-3 space-y-2 bg-white">
                  <div className="flex gap-1.5">
                    {(["fixed","percent"] as const).map(t => (
                      <button
                        key={t}
                        onClick={() => handleDiscountTypeToggle(t)}
                        className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${
                          discount.type === t ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-500"
                        }`}
                      >
                        {t === "fixed" ? "円引き" : "％引き"}
                      </button>
                  ...
                  {discount.type === "fixed" && (
                    <div className="flex gap-1.5">
                      {([true, false] as const).map(inc => (
                        <button
                          key={String(inc)}
                          onClick={handleDiscountInclusiveToggle}
                          className={`flex-1 py-1 rounded-lg text-[10px] font-semibold transition-all ${
                            discount.inclusive === inc ? "bg-orange-400 text-white" : "bg-slate-100 text-slate-400"
                          }`}
                        >
                          {inc ? "税込から" : "税抜から"}
                        </button>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between bg-slate-50 border border-orange-200 hover:border-orange-400 rounded-lg px-3 py-2.5 transition-colors group"
                  >
                    <span className={`text-base font-black tabular-nums ${discount.value > 0 ? "text-slate-800" : "text-slate-300"}`}>
                      {discount.value > 0
                        ? `${discount.value.toLocaleString()}${discount.type === "fixed" ? "円" : "%"}`
                        : "金額を入力…"}
                    </span>
                    <span className="text-slate-700 group-hover:text-orange-600 transition-colors flex-shrink-0">
                      <PenIcon />
                    </span>
                  </button>
                </div>
              ...
              <button
                ...
                className="w-full py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-black transition-all"
              >
                ✕ 割引を解除する
              </button>
            </div>

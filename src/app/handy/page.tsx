  ...
  return (
    <div className="min-h-[100svh] flex flex-col bg-slate-100 w-full overflow-x-hidden">
      <div className="flex-1 flex flex-col w-full max-w-md mx-auto">
        {/* ── Header ─────────────────────────────────────── */}
        <header className="bg-indigo-600 text-white shadow-md sticky top-0 z-30" style={{ paddingTop: "env(safe-area-inset-top)" }}>
          <div className="px-3 py-2 flex items-center gap-2 min-w-0">
            ...
            <select
              ...
              className="flex-shrink-0 bg-indigo-700 border border-indigo-400 rounded-lg px-1.5 py-1 text-xs font-bold outline-none cursor-pointer max-w-[80px]"
              aria-label="Language"
            >
              {HANDY_LANGS.map(l => (
            ...
            {([
              ...
              <button
                ...
                className={`flex-1 min-w-0 py-2 text-xs font-bold transition-all relative ${
                  tab === item.key ? "bg-white text-indigo-700" : "text-indigo-100"
                }`}
              >
                ...
                {filteredItems.map(item => {
                  ...
                  return (
                    <button key={item.id} onClick={() => addToCart(item)}
                      className="bg-white rounded-xl shadow-sm border border-slate-200 hover:border-indigo-400 active:scale-95 transition-all text-left overflow-hidden min-w-0">
                      {item.imageUrl && (
                      ...
                      <div className="p-2 min-w-0">
                        ...
                        <p className="text-sm font-black text-indigo-600 mt-1">¥{totalPrice.toLocaleString()}</p>
                      </div>
                    </button>
                  );
                })}
            ...
            {cart.length === 0 ? (
              ...
              <div className="flex gap-2 min-w-0">
                ...
                <button onClick={sendToKitchen} disabled={sending}
                  className="flex-[1.4] min-w-0 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white rounded-2xl font-black text-xs px-2 py-2.5 active:scale-95 transition-all truncate">
    ...
    <div className={`bg-white rounded-2xl shadow-sm border overflow-hidden min-w-0 ${
      ...
      {!readonly && !order.closed && (
        <div className="flex border-t border-slate-100">
          ...
          <button onClick={onClose} className="flex-1 py-2.5 text-xs font-bold text-indigo-600 hover:bg-indigo-50 active:bg-indigo-100 border-l border-slate-100">
            💴 {labels.close}
          </button>
